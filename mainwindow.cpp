...
void MainWindow::on_pushButton_2_clicked()
{
   ...
   else
   {
       ...
       QRecLevel lvl = ECLEVEL(MainWindow::er_correction);
       q = QRcode_encodeString(ui->lineEdit->text().toStdString().c_str(),\
...
//-----------------------------------//
    ...
    for(int y=0;y<q->width;y++)
        ...
        {
           double scale = 300.0/q->width; // The size of code.
           unsigned char b = q->data[y*q->width+x];
           if(b & 0x01) // If the last character of b is '0', then draw a black block.
           {
...
QRecLevel MainWindow::ECLEVEL(int lvl){
    switch (lvl){
    case 0:
        return QR_ECLEVEL_L;
    case 1:
